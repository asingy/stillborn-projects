...
$this->registerJs('$("#image-up-stamp").fileinput({
        language: "ru",
...
<?php $form = ActiveForm::begin(['id'=>'stamp-types-form','enableAjaxValidation' => true, 'validateOnSubmit' => true, 'options'=>['enctype'=>'multipart/form-data']]); ?>
      ...
      <div class="row">
          <div class="col-md-6" >
              ...
              <div class="thumbnail" style="padding:20px; height:180px; border-radius: 0px;">
                  ...
                  <?= isset($model->image) ? Html::img(Url::to(Yii::$app->request->baseUrl.'/images/stamp/'.$model->image, true), ['style'=>'max-width: 100%;max-height: 100%;']) : '' ?>
              </div>
              <input id="image-up-stamp" name="image" type="file" accept="image/*" class="file-loading">
          </div>
          <div class="col-md-6">
            <?= $form->field($model, 'name')->textInput(['maxlength' => 128]) ?>
            <?= $form->field($model, 'info')->textarea(['rows' => 3]) ?>
            <?= $form->field($model, 'status')->dropDownList(StampHelper::getAllStats(),[]) ?>
